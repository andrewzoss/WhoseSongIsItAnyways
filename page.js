...
function ResultsView({ league, round, roundIndex, roundScores, totals, isAdmin, currentPlayer, onBack, onOpenHistory, onAssign, onUnreveal }) {
  ...
  // Per-player diagnostic stats: how many guesses they recorded and how
  // many were correct. Helps spot data issues at a glance.
  const playerStats = {};
  for (const p of round.players) {
    const myGuesses = round.guesses?.[p] || {};
    const made = Object.keys(myGuesses).length;
    let correct = 0;
    for (const [tid, guess] of Object.entries(myGuesses)) {
      const truth = round.claims?.[tid];
      if (truth && String(guess).trim() === String(truth).trim() && truth !== p) correct += 1;
    }
    playerStats[p] = { made, correct };
  }
      ...
      <div className="ml-card">
        ...
        <div className="ml-scoreboard">
          {sortedThisRound.map((p, i) => {
            ...
            const stats = playerStats[p] || { made: 0, correct: 0 };
            return (
              <div key={p} className={`ml-score-row ${isWinner ? 'winner' : ''}`}>
                ...
                <div className="ml-score-name">
                  {isWinner && '🏆 '}{p}
                  <span style={{ marginLeft: 10, fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: 'var(--text-faint)', letterSpacing: '0.06em' }}>
                    {stats.correct}/{stats.made} right
                  </span>
                </div>
