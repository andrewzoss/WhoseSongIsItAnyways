...
// Defensive: normalize names with .trim() so subtle whitespace differences
// don't silently zero someone out.
...
export function scoreRound(round) {
  ...
  for (const t of round.tracks || []) {
    const truth = norm(round.claims?.[t.id]);
    if (!truth) continue;
    for (const p of players) {
      if (norm(p) === truth) continue;
      const g = norm(round.guesses?.[p]?.[t.id]);
      if (g && g === truth) scores[p] += 1;
    }
  }
  return scores;
}
